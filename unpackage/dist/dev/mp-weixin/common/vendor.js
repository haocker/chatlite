...
function validateProtocol(name, data, protocol2, onFail) {
  ...
  for (const key in protocol2) {
    const errMsg = validateProp(key, data[key], protocol2[key], !hasOwn(data, key));
    if (isString(errMsg)) {
      onFail(name, errMsg);
    }
  }
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__AI_CHAT_CLIENT",
    appName: "AI Chat Client",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function initRuntimeSocketService() {
  const hosts = "192.168.201.40,192.168.225.18,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_DD3mR5";
...
function checkSanitizeDeprecation(opt) {
  if (opt && opt.sanitize && !opt.silent) {
    index.__f__("warn", "at node_modules/marked/lib/marked.esm.js:279", "marked(): sanitize and sanitizer parameters are deprecated since version 0.7.0, should not be used and will be removed in the future. Read more here: https://marked.js.org/#/USING_ADVANCED.md#options");
  }
}
    ...
    while (src) {
      ...
      if (src) {
        ...
        if (this.options.silent) {
          index.__f__("error", "at node_modules/marked/lib/marked.esm.js:1696", errMsg);
          break;
        } else {
          throw new Error(errMsg);
        }
  ...
  inlineTokens(src, tokens = []) {
    ...
    while (src) {
      ...
      if (src) {
        const errMsg = "Infinite loop on byte: " + src.charCodeAt(0);
        if (this.options.silent) {
          index.__f__("error", "at node_modules/marked/lib/marked.esm.js:1882", errMsg);
          break;
        } else {
          throw new Error(errMsg);
        }
    ...
    for (i2 = 0; i2 < l4; i2++) {
      ...
      switch (token.type) {
        ...
        default: {
          ...
          if (this.options.silent) {
            index.__f__("error", "at node_modules/marked/lib/marked.esm.js:2379", errMsg);
            return;
          } else {
            throw new Error(errMsg);
          }
    ...
    for (i2 = 0; i2 < l2; i2++) {
      ...
      switch (token.type) {
        ...
        default: {
          const errMsg = 'Token with "' + token.type + '" type was not found.';
          if (this.options.silent) {
            index.__f__("error", "at node_modules/marked/lib/marked.esm.js:2458", errMsg);
            return;
          } else {
            throw new Error(errMsg);
          }
...
const pages = [
  {
    ...
    style: {
      navigationBarTitleText: "AI Chat Assistant",
      navigationBarBackgroundColor: "#4a5568",
...
const globalStyle = {
  ...
  navigationBarTitleText: "AI Chat Client",
